...
const loadChallengeConfig = async () => {
  ...
  console.log(`Loading challenge config from SSM: ${paramName}`);
...
const loadScorerConfig = async (scorerType) => {
  ...
  console.log(`Loading scorer config from SSM: ${paramName}`);
...
const getAccessToken = async () => {
  ...
  console.log(`Access token cached, expires in ${expiresIn} seconds`);
  return tokenCache;
};
 ...
 */
const launchEcsTask = async ({ submissionId, scorerType, challengeConfig, scorerConfig, accessToken }) => {
  ...
  console.log(`Launched ECS task ${taskArn} for submission ${submissionId}, scorer ${scorerType}`);
  return taskArn;
};
...
const processRecord = async (record) => {
  ...
  try {
    ...
    if (challengeId !== config.challengeId) {
      console.warn(`Message challenge ${challengeId} doesn't match configured challenge ${config.challengeId}`);
...
exports.handler = async (event) => {
  console.log(`Challenge Processor Lambda (${config.challengeId}) received ${event.Records?.length || 0} messages`);
...
(async () => {
  if (config.challengeId) {
    console.log(`Cold start: Pre-loading configurations for challenge ${config.challengeId}`);
    try {
      await loadChallengeConfig();
    } catch (error) {
